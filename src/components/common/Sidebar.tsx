...
import { useNavVisibility } from "@/store/permissions";
...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  // Falls back to "global-admin" (full access) when the logged-in user has
  // no role set yet, so nothing hides unexpectedly before that's wired up.
  const roleId = useAuthStore((s) => s.user?.role) ?? "global-admin";
  const isNavItemVisible = useNavVisibility(roleId);
  const visibleNavItems = filterNavItems(NAV_ITEMS, isNavItemVisible);
